...
impl Map {
    fn recalculate_bounds(&mut self) {
        ...
        self.min.3 = self.cells.iter().map(|k| k.3).min().unwrap_or_default();
        ...
        self.max.3 = self.cells.iter().map(|k| k.3).max().unwrap_or_default();
    }
    fn new(cells: HashSet<(i32, i32, i32, i32)>) -> Self {
        let mut res = Self {
            cells,
            min: (0, 0, 0, 0),
            max: (0, 0, 0, 0),
    ...
    for x in src.min.0 - 1..=src.max.0 + 1 {
        for y in src.min.1 - 1..=src.max.1 + 1 {
            for z in src.min.2 - 1..=src.max.2 + 1 {
                for w in src.min.3 - 1..=src.max.3 + 1 {
                    let live = src.cells.contains(&(x, y, z, w));
                    let mut count = 0;
                    for dx in -1..=1 {
                        for dy in -1..=1 {
                            for dz in -1..=1 {
                                for dw in -1..=1 {
                                    ...
                                    if src.cells.contains(&(x + dx, y + dy, z + dz, w + dw)) {
                                        count += 1;
                                    }
                                }
                            }
